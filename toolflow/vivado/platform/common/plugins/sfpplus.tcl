...
##### External (JSON) configuration format #####
...
namespace eval sfpplus {
  ...
  proc create_reciever_cell {port_name num_reciever} {
    set port [create_bd_cell -type hier SFP_${port_name}_reciever]
    set current [current_bd_instance .]
    current_bd_instance $port
  ...
  proc create_transmitter_cell {port_name num_sender} {
    set port [create_bd_cell -type hier SFP_${port_name}_transmitter]
    set current [current_bd_instance .]
    current_bd_instance $port
  ...
  # @param ic_sync if true, AXIS Interconnects are added to synchronize between the SFP+- and PE- clocks; otherwise synchronization must be handled by the PE
  proc connect_recievers {interface_list port_in port_name ic_sync} {
    for {variable i 0} {$i < [llength $interface_list]} {incr i} {
      ...
      puts [get_bd_intf_pins $port_in/M[format %02d $i]_AXIS]
      puts [get_bd_intf_pins $interface]
      connect_bd_intf_net [get_bd_intf_pins $port_in/M[format %02d $i]_AXIS] [get_bd_intf_pins $interface]

      if {!$ic_sync} {
        connect_clock_pe_sync $interface $port_in rx
      }
    }
  }
  ...
  # @param port_name the name of the port
  # @param ic_sync if true, AXIS Interconnects are added to synchronize between the SFP+- and PE- clocks; otherwise synchronization must be handled by the PE
  proc connect_senders {interface_list port_out port_name ic_sync} {
    for {variable i 0} {$i < [llength $interface_list]} {incr i} {
      set interface [lindex $interface_list $i]
      puts "Connecting sender $interface to Port $port_name"
      puts [get_bd_intf_pins $port_out/S[format %02d $i]_AXIS]
      puts [get_bd_intf_pins $interface]
      connect_bd_intf_net [get_bd_intf_pins $port_out/S[format %02d $i]_AXIS] [get_bd_intf_pins $interface]

      if {!$ic_sync} {
        connect_clock_pe_sync $interface $port_out tx
      }
    }
  }
  ...
  proc generate_broadcast {ic_sync num_reciever num_sender port_in port_out} {
    set current [current_bd_instance .]
    current_bd_instance $port_in

    # Create Reciever Interconnect
    set reciever [tapasco::ip::create_axis_broadcast reciever]
    set_property CONFIG.NUM_MI $num_reciever $reciever
    set_property -dict [list CONFIG.M_TDATA_NUM_BYTES {8} CONFIG.S_TDATA_NUM_BYTES {8}] $reciever

    for {variable i 0} {$i < $num_reciever} {incr i} {
        set_property CONFIG.M[format "%02d" $i]_TDATA_REMAP tdata[63:0] $reciever
    }
  ...
  proc generate_roundrobin {ic_sync num_reciever num_sender port_in port_out} {
    set current [current_bd_instance .]
    current_bd_instance $port_in
  ...
  proc generate_singular {ic_sync port_in port_out} {
    if {$ic_sync} {
      set current [current_bd_instance .]
      current_bd_instance $port_in

      # Interconnect for reciever synchronization
      ...
      current_bd_instance $port_out
      # Interconnect for transmitter synchronization
      ...
      # connect directly
      connect_bd_intf_net [get_bd_intf_pins $port_in/AXIS_RX] [get_bd_intf_pins $port_in/M00_AXIS]
      connect_bd_intf_net [get_bd_intf_pins $port_out/S00_AXIS] [get_bd_intf_pins $port_out/AXIS_TX]
    }
  }
  ...
  proc connect_clock_pe_sync {interface port direction} {
    ...
    foreach clk $clks {
      ...
      puts "Clk: $clk; Interface: $interfaces"
